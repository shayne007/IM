/**
 * Created by nien woo Date: 17-12-30 Time: 下午4:50
 */
...
public class IOUtil {
    ...
    /**
     * 取得当前类路径下的 resName资源的完整路径 url.getPath()获取到的路径被utf-8编码了 需要用URLDecoder.decode(path, "UTF-8")解码
     *
    ...
    /**
     * 构建当前类路径下的 resName资源的完整路径 url.getPath()获取到的路径被utf-8编码了 需要用URLDecoder.decode(path, "UTF-8")解码
     *
     * @param resName 需要获取完整路径的资源,需要以/打头
     * @return 完整路径
     */
    ...
    /**
     * ByteBuffer中字符串是以ascii中0（null）字符结束 如果是以别的字符结束，则修改if(btemp[i]==0){ break; }即可
     *
     * @param buf
     * @param MaxLen
     * @return
